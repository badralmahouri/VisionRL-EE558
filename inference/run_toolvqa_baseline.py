"""Baseline inference on the ToolVQA RL test split.

Supports Apertus and Qwen baselines on the same held-out ToolVQA text-answer
examples used by the ToolVQA RL checkpoint. Outputs prediction JSONL compatible
with evaluation/compute_accuracy.py --mode mcq.
"""
...
TOOLVQA_PROMPT_SUFFIX = "\n\nAnswer concisely with the final answer only."
...
def load_metadata(path: Path, split: str | None = "test", limit: int | None = None) -> list[dict[str, Any]]:
    ...
    with path.open(encoding="utf-8") as f:
        for line in f:
            ...
            if split and row.get("split") != split:
                continue
            row["answers"] = [str(row["answer"]).strip()]
            rows.append(row)
...
def build_apertus_prompt(image_token_str: str, question: str) -> str:
    return (
        ...
        "<|developer_end|>"
        "<|user_start|>"
        f"{image_token_str}\n{question}{TOOLVQA_PROMPT_SUFFIX}"
        "<|user_end|>"
        "<|assistant_start|>"
    )


def encode_apertus_images(metadata: list[dict[str, Any]], vq_model_path: str, device: str) -> dict[Any, str]:
    print(f"Loading VQ model from {vq_model_path} ...")
    vq_model = load_vq_model(vq_model_path, device=device)
    image_tokens: dict[Any, str] = {}
    start = time.time()
...
def apertus_worker(gpu_id: int, samples: list[dict[str, Any]], image_tokens: dict[Any, str], config: dict[str, Any], output_path: Path) -> None:
...
def qwen_worker(gpu_id: int, samples: list[dict[str, Any]], config: dict[str, Any], output_path: Path) -> None:
    ...
    for i, sample in enumerate(samples):
        ...
        messages = [
            {"role": "system", "content": "Answer the question concisely with the final answer only."},
            {
                "role": "user",
                "content": [
                    {"type": "image", "image": image},
                    {"type": "text", "text": f"{sample['question']}{TOOLVQA_PROMPT_SUFFIX}"},
                ],
            },
        ]
...
def run_parallel(model_name: str, metadata: list[dict[str, Any]], config: dict[str, Any], output_dir: Path) -> Path:
    ...
    merged.sort(key=lambda row: str(row["question_id"]))
...
def main() -> None:
    parser = argparse.ArgumentParser(description="ToolVQA baseline inference")
    parser.add_argument("--model", choices=["apertus", "qwen"], required=True)
    parser.add_argument("--config", required=True)
    parser.add_argument("--metadata", default="data_prep/toolvqa_rl/metadata.jsonl")
    ...
    output_dir = Path(args.output_dir) if args.output_dir else PROJECT_ROOT / "results" / "toolvqa_rl" / f"{args.model}_baseline"
    output_dir.mkdir(parents=True, exist_ok=True)

    metadata = load_metadata(metadata_path, split=args.split, limit=args.limit)
    print(f"Loaded {len(metadata)} ToolVQA samples from {metadata_path} split={args.split}")
    if not metadata:
        raise ValueError("No metadata rows loaded")
