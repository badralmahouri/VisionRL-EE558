"""Baseline inference on the synthetic rotate-flip test set.
...
APERTUS_SYSTEM_PROMPT = (
    ...
    "Answer with the exact text requested by the user."
)
ROTATE_FLIP_PROMPT_SUFFIX = "\n\nAnswer with only the correctly spelled lowercase word."
...
def load_metadata(path: Path, limit: int | None = None) -> list[dict[str, Any]]:
    ...
    with path.open(encoding="utf-8") as f:
        for line in f:
            if line.strip():
                row = json.loads(line)
                row["answers"] = [row.get("canonical_answer", row["answer"])]
                rows.append(row)
...
def build_apertus_prompt(image_token_str: str, question: str) -> str:
    return (
        ...
        "<|developer_end|>"
        "<|user_start|>"
        f"{image_token_str}\n{question}{ROTATE_FLIP_PROMPT_SUFFIX}"
        "<|user_end|>"
        "<|assistant_start|>"
    )
...
def qwen_worker(gpu_id: int, samples: list[dict[str, Any]], config: dict[str, Any], output_path: Path) -> None:
    ...
    for i, sample in enumerate(samples):
        ...
        messages = [
            {"role": "system", "content": "Answer with only the exact word, preserving capitalization."},
            {
                "role": "user",
                "content": [
                    {"type": "image", "image": image},
                    {"type": "text", "text": f"{sample['question']}{ROTATE_FLIP_PROMPT_SUFFIX}"},
                ],
            },
        ]
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Rotate-flip baseline inference")
    parser.add_argument("--model", choices=["apertus", "qwen"], required=True)
    parser.add_argument("--config", required=True)
    parser.add_argument("--metadata", default="data_prep/rotate_flip_rl/test_metadata.jsonl")
    ...
    output_dir = Path(args.output_dir) if args.output_dir else PROJECT_ROOT / "results" / "rotate_flip_rl" / f"{args.model}_baseline"
    output_dir.mkdir(parents=True, exist_ok=True)

    metadata = load_metadata(metadata_path, args.limit)
    print(f"Loaded {len(metadata)} rotate-flip samples from {metadata_path}")
