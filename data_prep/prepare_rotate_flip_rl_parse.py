"""Parse raw rotate_flip_tool data into Apertus RL parquet.

This is the GPU/Apertus stage. It loads raw.jsonl from
prepare_rotate_flip_rl_generate.py, encodes images with Emu3.5/IBQ, and writes
...
Example:
    ...
    python data_prep/prepare_rotate_flip_rl_parse.py --limit 8
"""
...
ROTATE_FLIP_RL_INSTRUCTION = (
    "Use the rotate_flip_tool if the text is sideways, upside down, or mirrored. "
    "The tool can rotate or flip the image and returns a transformed image. "
    "Then answer with the correctly spelled lowercase word represented by the sign text. "
    "Call the display_answers tool exactly once at the end of your response."
)
ROTATE_FLIP_RL_NO_TOOL_INSTRUCTION = (
    "Answer the visual question directly from the image. Return only the correctly "
    "spelled lowercase word represented by the sign text."
)
...
def build_user_content(question: str, image_token_str: str, no_tool: bool = False) -> str:
    instruction = ROTATE_FLIP_RL_NO_TOOL_INSTRUCTION if no_tool else ROTATE_FLIP_RL_INSTRUCTION
...
def build_parquet_record(
    ...
    target_answer = meta.get("canonical_answer", meta["answer"])
    extra_info = {
        "index": meta["question_id"],
        "split": split,
        "answer": meta["answer"],
        "canonical_answer": target_answer,
        "transform": meta.get("transform"),
        "inverse_operations": meta.get("inverse_operations"),
    ...
    return {
        ...
        "ability": "spatial_rotate_flip_reasoning_no_tool" if no_tool else "spatial_rotate_flip_reasoning",
        "reward_model": {"style": "rule", "ground_truth": target_answer},
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Parse rotate-flip RL data into Apertus parquet")
    parser.add_argument("--input", default=None, help="Default: data_prep/rotate_flip_rl/raw.jsonl")
    parser.add_argument("--test-input", default=None, help="Default: data_prep/rotate_flip_rl/test_raw.jsonl if it exists")
